...
The LXC container that Proxmox creates is called `docker`. This isn't super helpful when running multiple containers in the same Proxmox instance. Each LXC container has a configuration file in `/etc/pve/lxc`, named after the container ID. Let's modify the file for our `docker` container:
