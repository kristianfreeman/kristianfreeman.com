...
While not technically a "Message of the Day", you can also put a message in your shell's profile. This is a file that is executed when you start a new shell. You can confirm this by running `echo $PS1` in a shell.

```bash
$ echo "Welcome to my machine!" > ~/.profile
```

You could even replicate the functionality of `/etc/motd` by creating `~/.motd` and then adding the following to your `~/.profile`:
...
An SSH banner is a file that is displayed when you SSH into a machine. This is a great way to display a message that you want to show to all users.

To set this up, you need to create a file called `~/.ssh/banner` and add the following to it:
