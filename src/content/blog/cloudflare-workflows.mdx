...
Imagine that we want to build a workflow that fetches analytics for a SaaS product we're building. It will then take those analytics and report them to a private Slack channel. We can break this down into two steps:

1. Fetch analytics from our SaaS product
2. Report those analytics to a private Slack channel
  ...
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    ...
    await step.do('report analytics', async () => {
      const resp = await fetch('https://slack.com/api/chat.postMessage', {
        method: 'POST',
        body: JSON.stringify({
          channel: event.params.channelId,
